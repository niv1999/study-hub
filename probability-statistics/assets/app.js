/* =====================================================================
   Intro to Artificial Intelligence — Study Site · app.js
...
(function () {
  ...
  var PROGRESS_KEY = 'stats-progress';
  var TOTAL_MODULES = 18; // per MODULE_MAP (15 content + exam prep + practice exams)
  ...
  /* =====================================================================
     §7.3b2  exam answer checks (CONTRACT §9 — 15-real-exams + practice exams)
     .exam-answer[data-answer][data-tol]: the "בדוק" button compares the
     numeric input to data-answer within ±data-tol and shows ✓/✗ in .exam-mark.
     ===================================================================== */
  function initExamAnswers() {
    var boxes = document.querySelectorAll('.exam-answer[data-answer]');
    if (!boxes.length) return;
    injectCSS('stats-exam-answer-css',
      '.exam-answer{display:flex;flex-wrap:wrap;align-items:center;gap:.55rem;margin:.9rem 0;' +
        'padding:.6rem .85rem;background:var(--surface-2,#F6F2EA);border:1px solid var(--line,#E4DCCC);border-radius:10px}' +
      '.exam-answer label{font-weight:600;font-size:.92rem;margin:0}' +
      '.exam-answer input{direction:ltr;width:8.5rem;padding:.4rem .6rem;border:1px solid var(--line,#CFC6B4);' +
        'border-radius:8px;font-family:"JetBrains Mono",ui-monospace,monospace;font-size:.95rem;' +
        'background:var(--surface,#fff);color:inherit}' +
      '.exam-answer .check-btn{padding:.38rem .95rem;border:1px solid var(--accent,var(--dusty-blue,#7E9BA6));' +
        'background:transparent;color:var(--accent,inherit);border-radius:999px;cursor:pointer;font-weight:600;font-size:.9rem}' +
      '.exam-answer .check-btn:hover{background:var(--accent,var(--dusty-blue,#7E9BA6));color:#fff}' +
      '.exam-mark{font-weight:700;font-size:.95rem;min-inline-size:5.5rem}' +
      '.exam-mark.ok{color:#2E7D46}.exam-mark.bad{color:#B3402E}' +
      'html[data-theme="dark"] .exam-mark.ok{color:#8FD4A0}' +
      'html[data-theme="dark"] .exam-mark.bad{color:#F0A08E}' +
      '.exam-q{background:var(--surface,#fff);border:1px solid var(--line,#E4DCCC);border-radius:14px;' +
        'padding:1.1rem 1.25rem;margin:1.2rem 0}' +
      '.exam-q>h3:first-child,.exam-q>h4:first-child{margin-top:0}' +
      '.exam-summary{background:var(--surface-2,#F6F2EA);border:1px solid var(--line,#E4DCCC);' +
        'border-radius:14px;padding:1rem 1.25rem;margin:1.4rem 0}' +
      '.exam-summary label{display:flex;align-items:center;gap:.5rem;margin:.35rem 0;cursor:pointer;font-size:.95rem}');
    for (var i = 0; i < boxes.length; i++) {
      (function (box) {
        var input = box.querySelector('input');
        var btn = box.querySelector('.check-btn');
        var mark = box.querySelector('.exam-mark');
        if (!input || !btn || !mark) return;
        var answer = parseFloat(box.getAttribute('data-answer'));
        var tol = parseFloat(box.getAttribute('data-tol'));
        ...
        function check() {
          ...
          if (raw === '' || isNaN(val)) {
            mark.textContent = 'הזינו מספר';
            mark.className = 'exam-mark';
            return;
          }
          var ok = Math.abs(val - answer) <= tol;
          mark.textContent = ok ? '✓ נכון' : '✗ לא מדויק';
          mark.className = 'exam-mark ' + (ok ? 'ok' : 'bad');
        }
